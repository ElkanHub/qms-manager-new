...
type GrainientCtx = {
  renderer: InstanceType<typeof import('ogl').Renderer>
  program: InstanceType<typeof import('ogl').Program>
  mesh: InstanceType<typeof import('ogl').Mesh>
}
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true
      const cleanup = (container as any)?.__grainientCleanup
      if (cleanup) cleanup()
    }
